...
public class Webcam {
	...
	/**
	 * Lock which denies access to the given webcam when it's already in use by other webcam capture API process or thread.
	 */
	...
	/**
	 * Open the webcam in either blocking (synchronous) or non-blocking (asynchronous) mode. If the non-blocking mode is enabled the
	 * DefaultDelayCalculator is used for calculating delay between two image fetching.
	...
	/**
	 * Open the webcam in either blocking (synchronous) or non-blocking (asynchronous) mode.The difference between those two modes lies in the image acquisition mechanism.<br>
	 * <br> In blocking mode, when user calls {@link #getImage()} method, device is being queried for new image buffer and user have to wait for it to be available.<br>
	 * <br> In non-blocking mode, there is a special thread running in the background which constantly fetch new images and cache them internally for further use. This cached instance is returned every time when user request new
	 * image. Because of that it can be used when timeing is very important, because all users calls for new image do not have to wait on device
	 * response. By using this mode user should be aware of the fact that in some cases, when two consecutive calls to get new image are executed more
	 * often than webcam device can serve them, the same image instance will be returned. User should use {@link #isImageNew()} method to distinguish if
	 * returned image is not the same as the previous one. <br> The background thread uses implementation of DelayCalculator interface to
	 * calculate delay between two image fetching. Custom implementation may be specified as parameter of this method. If the non-blocking mode is
	 * enabled and no DelayCalculator is specified, DefaultDelayCalculator will be used.
	 * @param async true for non-blocking mode, false for blocking
	 * @param delayCalculator responsible for calculating delay between two image fetching in non-blocking mode; It's ignored in blocking mode.
	...
	/**
	 * Return underlying webcam device. Depending on the driver used to discover devices, this method can return instances of different class. By default {@link WebcamDefaultDevice} is
	...
	/**
	 * Capture image from webcam and return it. Will return image object or null if webcam is closed or has been already disposed by JVM.<br> <br>
	 * <b>IMPORTANT NOTE!!!</b><br> <br> There are two possible behaviors of what webcam should do when you try to get image and
	 * webcam is actually closed. Normally it will return null, but there is a special flag which can be statically set to switch all webcams to auto open mode. In this mode, webcam will be
	 * automatically open, when you try to get image from closed webcam. Please be aware of some side effects! In case of multi-threaded applications, there is no guarantee that one thread 
	...
	/**
	 * Get RAW image ByteBuffer. It will always return buffer with 3 x 1 bytes per each pixel, where RGB components are on (0, 1, 2) and color space is sRGB.<br> <br>
	 * <b>IMPORTANT!</b><br> Some drivers can return direct ByteBuffer, so there is no guarantee that underlying bytes
	 * will not be released in next read image operation. Therefore, to avoid potential bugs you should convert this ByteBuffer to bytes array before you fetch next image.
	 * @return Byte buffer
	 */
	...
	/**
	 * Get RAW image ByteBuffer. It will always return buffer with 3 x 1 bytes per each pixel, where RGB components are on (0, 1, 2) and color space is sRGB.<br>
	 * <br> <b>IMPORTANT!</b><br> Some drivers can return direct ByteBuffer, so there is no guarantee that underlying bytes
	 * will not be released in next read image operation. Therefore, to avoid potential bugs you should convert this ByteBuffer to bytes array before you fetch next image.
	...
	public void getImageBytes(ByteBuffer target) {
		...
		assert driver != null;
		assert device != null;
		// some devices can support direct image buffers, and for those call processor task, and for those which does not support direct image buffers, just convert image to RGB byte array
		if (device instanceof BufferAccess) {
	...
	/**
	 * Set new video driver class to be used by webcam. Class given in the argument shall extend {@link WebcamDriver} interface and should have public default constructor, so instance can be created by reflection.<br> <br>
	...
	/**
	 * <b>CAUTION!!!</b><br> <br>
	 * This is experimental feature to be used mostly in in development phase. After you set handle term signal to true, and fetch capture devices, Webcam Capture API will listen for TERM
	 * signal and try to close all devices after it has been received. <b>This feature can be unstable on some systems!</b>
	...
	/**
	 * Switch all webcams to auto open mode. In this mode, each webcam will be automatically open whenever user will try to get image from instance which has not yet been open. Please be
	 * aware of some side effects! In case of multi-threaded applications, there is no guarantee that one thread will not try to open webcam even if it was manually closed in different thread. 
	...
	/**
	 * Is auto open mode enabled. Auto open mode will will automatically open webcam whenever user will try to get image from instance which has not yet been open. Please be aware of some side
	 * effects! In case of multi-threaded applications, there is no guarantee that one thread will not try to open webcam even if it was manually closed in different thread.
