...
class WebcamPanel extends JPanel implements WebcamListener, PropertyChangeListener {
	...
	/**
	 * Metodo realizzato per risolvere il problema di casting di una variabile con livello di precisione più alto ad una con livello di precisione più basso.
	 */
	...
	public enum DrawMode {
		/**
		 * Do not resize image - paint it as it is. This will make the image to go off out the bounds if panel is smaller than image size.
		 */
		NONE,
		/**
		 * Will resize image to the panel bounds. This mode does not care of the image scale, so the final image may be disrupted.
		 */
		FILL,
		/**
		 * Will fir image into the panel bounds. This will resize the image and keep both x and y scale factor.
		 */
		FIT,
	}
	/**
	...
	public void webcamOpen(WebcamEvent we) {
		// if default size has not been provided, then use the one from webcam device (this will be current webcam resolution)
		if (defaultSize == null) {
